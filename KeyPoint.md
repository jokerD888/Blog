...
传统的卷积神经网络（CNN）在计算机视觉任务中表现优异，但它们在处理长程依赖和全局上下文信息方面存在一定限制。**ViT 旨在将纯的Transformer 模型引入视觉任务**，而不是将卷积神经网络中的某些卷积替换为自注意力，以利用其在自然语言处理（NLP）中的成功。

### 2. **模型架构**

ViT 的核心思想是将输入图像划分为多个固定大小的图像块（patch），然后将这些块展平并嵌入到一个线性空间中，以生成输入序列。这些步骤包括：

- **图像分块**：将输入图像划分为 N 个 P×P 的图像块，论文中是16×16，原始图片的尺寸为224×224，这样N就是196。
- **线性嵌入**：将每个图像块展平并通过线性变换，映射到固定维度。
- **位置编码**：为每个图像块添加位置编码，以保持空间信息。

### 3. **Transformer 编码器**

ViT 采用标准的 Transformer 编码器架构，包括多个自注意力层和前馈网络。自注意力机制允许模型关注不同图像块之间的关系，从而捕捉长程依赖。

### 4. **训练与数据集**

ViT 使用了大量的数据集（如 ImageNet）进行训练，以验证其性能。研究表明，Transformer 模型在大规模数据上具有更好的泛化能力。

### 5. **性能**

ViT 在许多视觉任务（如图像分类、目标检测等）上与传统的 CNN 竞争，甚至在某些情况下超越了它们。研究表明，ViT 在足够大的数据集上表现更好，且对数据的规模敏感。

### 6. **结论与未来工作**

论文总结了 ViT 的优势，并指出其在视觉任务中的潜力。同时，提出了未来可以探索的方向，包括对图像块大小、模型规模和预训练策略的进一步研究。

### 7. **重要性**

ViT 的提出开创了视觉 Transformer 的研究热潮，推动了计算机视觉领域的进步。许多后续工作基于 ViT 的思想，探索了各种改进和变体。

通过以上内容，可以看出 ViT 是如何将 Transformer 架构引入计算机视觉领域的，并展示了其在不同任务中的有效性和潜力。
...
首先Transformer是一个纯基于注意力机制的编码器和解码器，BERT使用了一个Transformer编码器，拓展到更一般的NLP任务上，使用了完形填空的自监督训练机制。ViT模型可以任务是将Transformer用到CV上，它将每个图片割成很多16×16的小方块，放入Transformer做训练，而现在讲的MAE可以认识是BERT的一个CV版本，同时基于ViT这篇文章，但是MAE将整个训练扩展到没有标号的数据上面，和BERT一样，通过完型填空来获取图片的一个理解。
