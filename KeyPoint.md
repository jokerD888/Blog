...
2. **空间金字塔池化**：在这个特征图上，SPP 使用不同尺度的窗口进行池化操作。例如，使用不同尺寸的窗口分别进行池化，分成 1×1、2×2、4×4 等不同的格子。这些池化窗口将整个特征图划分为不同的网格区域，在每个区域内通过最大池化或平均池化得到特征值。
...
- 步骤：
  1. **候选区域生成**：使用选择性搜索生成一系列可能的目标候选区域（region proposals，或称为提议区域）。
...
- 步骤：
  ...
  3. **区域池化**：使用 ROI Pooling 将候选区域映射到特征图上，得到**固定大小的特征向量，便于连结后输出**。
...
1. 将**输入图像划分为固定大小的图像块**，通常大小为16x16像素。
   例如，对于224x224大小的图像，划分成16x16的patches后，每个图像会有14x14=196个块。
2. 将每个图像块**展平为一维向量**，然后通过一个线性投影（Linear Projection）将这些展平的块映射到一个固定的维度。
...
在计算机视觉，迁移学习非常常见，于是**BERT想要在NLP做到像计算机视觉那样的迁移学习**，BERT针对微调设计。BERT之前的两个工作，ELMo使用双向的网络，但网络架构用的比较老的RNN，另一个是GPT，用了Transformer架构，但只能处理单向的信息，于是BERT模型建立在**只有编码器的Transformer**（因为编码器是双向的，解码器是单向的）上，通过双向训练能够同时利用上下文信息。
...
- **双向上下文理解**：与之前的预训练方法如Word2Vec或GloVe等只考虑一个方向（从左到右或者从右到左）不同，BERT利用Transformer架构中的自注意力机制实现了**深度双向训练**。这意味着在预训练过程中，每个词都能看到句子中所有的其他词，从而更好地捕捉词语间的复杂关系和语义信息。
- **Masked Language Model (MLM)**：这是BERT预训练过程中的关键任务之一。在这个任务中，输入序列中的一些单词会被随机掩盖起来，然后模型尝试根据上下文来预测这些被掩盖的单词。这种方法使得模型能够**学习到更加丰富的上下文表示**。
- **Next Sentence Prediction (NSP)**：除了MLM外，BERT还引入了下一个句子预测的任务，以**帮助模型理解句子之间的关系**。给定两个句子A和B，模型需要判断B是否为A之后的实际后续句子。这有助于提升模型在涉及多句逻辑推理的任务上的表现。
...
4. **Patch Merging**
   类似于 CNN 的池化操作，Swin Transformer 在每个阶段结束时，通过下采样（patch merging）来减小特征图尺寸，并增大通道数。具体的：取四个相邻的Patch，将这四个相邻Patch的特征拼接起来，使用一个线性投影层（通常是一个全连接层）将拼接后的特征投影到一个新的维度，那么特征图的尺寸会减少一半（高和宽分别减半），而特征的通道数则变为两倍。
...
- **有标签数据的损失**：采用传统的交叉熵损失。
- **无标签数据的损失**：通过**最小化学生模型与教师模型在无标签数据上的预测一致性**来实现，通常使用均方误差（MSE）度量。
...
- 计算模型在强增强后的图片上的**预测与伪标签的一致性损失**。在FixMatch中，这一损失是用**交叉熵损失**来衡量的。
...
包含多个阶段的训练：在**第一阶段**，使用所有标注数据训练目标检测器（即Teacher Model）直到收敛；随后在**第二阶段**，用训练好的检测器去预测无标注图片上的目标边框及其类别标签，再将confidence-based filtering应用在NMS处理后的每一个预测框上，用高阈值来获取高质量的伪标签（pseudo label）；在**第三阶段**，对每一个无标注图片应用强数据增广，随后让模型在标注数据和带伪标签的无标注数据上再次训练。
...
### MoCo：一种高效的无监督视觉表示学习方法

Facebook AI提出的Momentum Contrast（MoCo）方法是一种面向大规模图像数据的无监督对比学习框架，在不依赖标签的情况下通过**对比学习**的方式取得了出色的效果。MoCo的设计旨在解决动态字典编码、特征对比等难题。

![image-20241112230310795](./KeyPoint.assets/image-20241112230310795.png)

### 1. MoCo的核心思想

MoCo 的核心是构建一个动态的、对比学习中的“队列”（queue）和“动量编码器”（momentum encoder），通过队列增大动态字典的大小，通过动量编码器使得训练时特征尽可能保持一致性。它通过以下几个关键步骤实现：

- **动量编码器**：MoCo 通过两个编码器来计算特征，一个是“查询编码器” ($f_q$)上图绿色的encoder，另一个是“键值编码器” ($f_k$)上图蓝色的momentum encoder。查询编码器用于生成当前 batch 的查询样本表示，键值编码器生成被对比的样本表示。这两个编码器的参数并非独立训练，而是通过“动量更新”的方式逐渐变化：键值编码器参数是查询编码器参数的指数平均，这样可以稳定特征生成，减少表示抖动。
- **动态队列**：MoCo 使用一个动态队列存储负样本对，这个队列不断加入新的键值编码器生成的样本表示，同时移出旧的样本表示。相比于直接依赖 batch 内的样本对，这种机制可以提供一个更大的负样本池，有助于学习出更好的特征表示。

### 2. MoCo 的流程

整个 MoCo 训练流程如下：

1. **输入增强**：对一张输入图像进行两次不同的数据增强，得到两个视角的图像（$x^{query}$ 和 $x^{key}_0$）。
2. **查询和键值特征生成**：用查询编码器$f_q$编码$x^{query}$，得到查询特征 q；用键值编码器$f_k$编码 $x^{key}_0$，得到键值特征 k。
3. **动量更新**：通过动量机制更新键值编码器$f_k$的参数，使得它逐渐“跟随”查询编码器，但变化更平稳。
4. **对比学习**：计算查询特征 q 与当前队列中的所有键值特征的相似度（例如余弦相似度），将 q 与它对应的正样本 k 的相似度作为“正对比”，将 q 与队列中的其他键值特征的相似度作为“负对比”，并应用对比损失函数（例如 InfoNCE）。
5. **队列更新**：将 k 加入队列并移出最旧的键值特征，保持队列的长度固定。

### 3. MoCo 的优势

- **较大规模的负样本**：通过队列机制，MoCo 可以维持一个动态且较大的负样本集，有助于模型学习更加辨别性的表示。
- **平稳的特征表示**：动量更新机制使得键值编码器变化平稳，避免因查询编码器参数更新导致的特征漂移问题。
- **灵活性和高效性**：MoCo 不依赖于 batch size，适合在相对较小的 GPU 环境中训练。

### 4. 实验结果

MoCo 在 ImageNet 上进行了自监督预训练，之后进行微调，取得了与有监督学习相近的分类性能，并在多种下游任务（如目标检测和分割）中展现了良好的迁移能力，表明了该方法学习的表征在各种视觉任务中具备很好的泛化性。

### 5. 总结

MoCo 是一种有效的对比学习框架，通过动量编码器和动态队列解决了无监督学习中的关键问题，尤其适合大规模数据和资源受限的环境，在视觉任务中具有广泛的应用潜力。
